...
-- Policy: Only owners (or parents of owners) can update their profile
-- For simplicity in family app: Authenticated users can update
CREATE POLICY "Users can update gamification profiles"
    ON gamification_profiles FOR ALL
    USING (true);

-- Index for fast lookup by student
CREATE INDEX idx_gamification_student ON gamification_profiles(student_id);
