...
import { analizarFichaCompleta } from '../services/evaluacion-service'; // Importar servicio de evaluación
...
import { getCompetencyName } from '../utils/competencyHelpers'; // Servicio de Gamificación
import { ProgresoService } from '../services/progreso-service'; // Persistencia local
import { getTemasDisponiblesPorCurso } from '../services/khan/khan-por-curso.js'; // Importar temas dinámicos
...
const WorksheetGenerator = () => {
    ...
    useEffect(() => {
        if (!loading && subjects.length > 0) {
            ...
            if (paramTopic) {
                ...
                // Try to auto-select the subject if the topic matches a subject name (e.g. "Matemáticas")
    ...
    const handleGenerate = async () => {
        ...
        console.log('🔍 DEBUG - activityType:', activityType);
        console.log('🔍 DEBUG - topic:', topic);
        console.log('🔍 DEBUG - selectedSubject:', selectedSubject.name);
        console.log('🔍 DEBUG - numQuestions:', numQuestions);

        setGenerating(true);
        setGeneratedContent(null);

        try {
            // ✨ DETECTAR SI DEBE USAR GENERADOR DETERMINISTA (4º Primaria)
            ...
            if (useDeterministic) {
                console.log('🎯 Usando generador DETERMINISTA (100% fiable)');

                // Generar con código puro (Matemáticas o Geografía 4º Primaria)
                ...
                console.log('📦 DEBUG - Contenido generado:', deterministicResult);
                console.log('📊 DEBUG - Secciones:', deterministicResult?.sections);
                console.log('📋 DEBUG - Primera pregunta:', deterministicResult?.sections?.[0]?.questions?.[0]);
                    ...
                    if (newRows.length > 0) {
                        supabase.from('question_bank_local').insert(newRows)
                            .then(({ error }) => {
                                ...
                                else console.log(`✅ Saved ${newRows.length} qs (deterministic) to bank.`);
                            });
                    }
                }

                console.log('✅ Generación determinística completada');
                return; // Salir, no usar AI
            }

            // ========================
            // GENERACIÓN CON AI (Caso normal)
            // ========================
    ...
    const handleLocalCorrection = async (userAnswers, setCorrectionResult, timeSpent) => {
        ...
        generatedContent.sections?.forEach(sec => {
            sec.questions?.forEach(q => {
                allQuestions.push({
                    ...q,
                    subject: selectedSubject.name, // CRÍTICO: Para activar "Modo Inglés"
                    pregunta: q.text,
        ...
        // Realizar análisis avanzado (Async Update) con ID de estudiante para tracking
        ...
        if (selectedStudent?.id) {
            const evaluationData = result.analisisDetallado.map((item, index) => {
                // Enriquecer competencias con lo detectado por el motor de errores inglés
                let dynamicCompetencias = [...(allQuestions[index].competencias || [])];

                // Si el motor de errores detectó una competencia específica (ej. CP por error de plurales)
                ...
                return {
                    ...
                    errorPatternId: item.patronError || null // Guardamos el ID del patrón si existe (ej. DESC_HAVE_AGE)
                };
            });
            ...
            const worksheetInfo = {
                ...
                advancedMetrics: result.metricasAvanzadas // Pasar la telemetría neuro-pedagógica
            };
            ...
            try {
                ...
                // Inyectar datos de gamificación en el resultado visual
            ...
            // Calculamos el % de éxito por competencia usando los datos enriquecidos
            ...
            }).filter(c => c.count > 0).sort((a, b) => b.percentage - a.percentage); // Ordenar por éxito
            ...
            result.analisisDetallado.forEach((res, idx) => {
                ...
                if (res.feedbackEstructurado && res.feedbackEstructurado.nivel) {
                    ...
                    // Si falló y no hay medalla (error normal), es tema a repasar
            ...
            // 💾 PERSISTENCIA: Guardar progreso cumulativo localmente
            ...
            // Añadir info al reporte de misión
            ...
            if (huboLevelUp) {
                missionSummary.mensajePersonalizado = `¡🎉 LEVEL UP! Ahora eres un ${infoNivel.nombre}. ¡Increíble! 🚀`;
            } else if (missionSummary.medallas.gold > totalMedallas * 0.8 && totalMedallas > 0) {
                missionSummary.mensajePersonalizado = `¡WOW! Has volado más allá de las estrellas. ¡Eres un 'Go Far!' Master! 🚀🌟 (XP: ${progresoTotal.puntosTotales})`;
            } else if (missionSummary.medallas.silver > missionSummary.medallas.bronze) {
                missionSummary.mensajePersonalizado = `¡Subida de nivel! Estás convirtiendo el bronce en plata. ¡Sigue así! 💪 (XP: ${progresoTotal.puntosTotales})`;
            } else {
                missionSummary.mensajePersonalizado = `¡Misión cumplida! Cada error es una pista para mejorar. 🛡️ (XP: ${progresoTotal.puntosTotales})`;
            }
    ...
    const getAvailableTopics = () => {
        ...
        // Helper para contar palabras de categorías
        const getVocabCount = (keys) => {
            ...
            keys.forEach(k => {
                ...
                // UI: "Food & Drinks" -> JSON: "food" (Drinks no existe como key, está en Food?)
                // Ajustamos keys según 'deterministic-integration.js'
                let jsonKey = k;
        ...
        const KHAN_EXERCISES_POR_CURSO = {
            '4º Primaria': {
                ...
                'Lengua Castellana': ['Ortografía', 'Comprensión Lectora', 'Gramática', 'Palabras Agudas, Llanas y Esdrújulas', 'El Verbo - Tiempos Verbales', 'Sustantivos y Adjetivos', 'La Oración Simple', 'Tipos de Texto', 'Unidad 5: Prefijos, Numerales y Adjetivos con V', 'Unidad 6: Sufijos, Verbos y Ger/Gir', 'Repaso Examen Mañana: Temas 5 y 6'],
                ...
                'Inglés': [
                    // VOCABULARY (Individual Categories from JSON)
                    `Vocabulary: Sports (${getVocabCount(['sports'])} words)`,
                    `Vocabulary: Hobbies (${getVocabCount(['hobbies'])} words)`,
                    `Vocabulary: Colors (${getVocabCount(['colors'])} words)`,
                    `Vocabulary: Family (${getVocabCount(['family'])} words)`,
                    `Vocabulary: Body (${getVocabCount(['body'])} words)`,
                    `Vocabulary: Animals (${getVocabCount(['animals'])} words)`,
                    `Vocabulary: School (${getVocabCount(['school'])} words)`,
                    `Vocabulary: Food (${getVocabCount(['food'])} words)`,
                    `Vocabulary: House (${getVocabCount(['house'])} words)`,
                    `Vocabulary: Clothes (${getVocabCount(['clothes'])} words)`,
                    `Vocabulary: Nature (${getVocabCount(['nature'])} words)`,
                    `Vocabulary: Transport (${getVocabCount(['transport'])} words)`,
                    `Vocabulary: Numbers (${getVocabCount(['numbers'])} words)`,
                    `Vocabulary: Time (${getVocabCount(['time'])} words)`,
                    `Vocabulary: Daily Routines (${getVocabCount(['daily_routines'])} words)`,
                    `Vocabulary: Prepositions Place (${getVocabCount(['prepositions_place'])} words)`,
                    `Vocabulary: Prepositions Time (${getVocabCount(['prepositions_time'])} words)`,
                    `Vocabulary: City (${getVocabCount(['city'])} words)`,
                    `Vocabulary: Professions (${getVocabCount(['professions'])} words)`,

                    // GRAMMAR
                    'Grammar: Present Simple',
                    'Grammar: Present Continuous',
                    'Grammar: Past Simple (Was/Were & Verbs)',
                    'Grammar: Verb To Be (Am/Is/Are)',
                    'Grammar: Verb Have Got',
                    'Grammar: There is / There are',
                    'Grammar: Prepositions (Place & Time)',
                    'Grammar: Adjectives (Comparatives & Superlatives)',
                    'Grammar: Adverbs (Frequency)',
                    'Grammar: Modals (Can/Must)',
                    'Grammar: Countable & Uncountable',
                    'Grammar: Articles & Quantifiers',
                    'Grammar: Possessives & Saxon Genitive',
                    'Grammar: Question Words (Wh-)',

                    // SKILLS & CHALLENGES
                    'Skill: Sentence Building (Word Order)',
                    'Skill: Mixed Tenses Challenge',
                    'Skill: Translation Practice (Reverse)'
                ]
            }
        };
                        ...
                        {selectedSubject && (
                            <div className="bg-white rounded-2xl shadow-sm border border-indigo-200 p-6">
                                ...
                                <p className="text-xs text-slate-500 mt-2 italic">
                                    Se guardará para esta asignatura y se usará automáticamente.
                                </p>
                            </div>
                        )}
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="bg-purple-50 p-6 rounded-xl border border-purple-100">
                                    ...
                                    <div className="grid md:grid-cols-2 gap-4">
                                        ...
                                        >
                                            ...
                                            <div>
                                                <span className={`block font-bold ${enableTimer ? 'text-purple-900' : 'text-gray-600'}`}>Cronómetro</span>
                                        ...
                                        >
                                            ...
                                            <div>
                                                ...
                                                <span className="text-xs text-gray-500">Modo desafío (Examen)</span>
                                            </div>
                                ...
                                <div className="bg-blue-50 border-2 border-blue-200 rounded-xl p-4">
                                    <p className="text-sm text-blue-800 font-medium flex items-center gap-2">
                                        <span className="text-xl">💡</span>
                                        <span>¿No entiendes el tema? <button onClick={() => window.location.href = '/tutor'} className="font-bold underline hover:text-blue-600">Pregunta al Tutor IA</button></span>
                                    </p>
                                </div>
                                ...
                                {/* Dificultad */}
                                <div>
                                    <label className="block text-sm font-bold text-gray-700 mb-3">📊 Dificultad</label>
                                    <div className="grid grid-cols-3 gap-3">
                                        {DIFFICULTIES.map(diff => (
                                            <button
                                                key={diff}
                                                onClick={() => setDifficulty(diff)}
                                                className={`p-3 border-2 rounded-xl transition-all text-center ${difficulty === diff
                                            ...
                                            >
                                                <div className="font-black text-sm uppercase">{diff}</div>
                                            </button>
                                        ))}
                                    </div>
                                </div>
                                ...
                                <div>
                                    <label className="block text-sm font-bold text-gray-700 mb-3">🎯 Número de Ejercicios</label>
                                    <div className="grid grid-cols-4 gap-3">
                                        {[
                                            { value: 10, label: 'Rápido', desc: '10 ejercicios' },
                                ...
                                >
                                    ...
                                    {generating ? 'Preparando Entrenamiento...' : '✨ Comenzar Entrenamiento'}
                                </button>
